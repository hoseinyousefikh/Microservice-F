...
namespace Catalog_Service.src._01_Domain.Core.Contracts.Repositories
{
    public interface IProductTagRepository
    {
        ...
        Task<ProductTag> AddAsync(ProductTag tag, CancellationToken cancellationToken = default);
        void Remove(ProductTag tag);
        Task<int> SaveChangesAsync(CancellationToken cancellationToken = default);

        // متدهای جستجو و فیلتر
        Task<(IEnumerable<ProductTag> Tags, int TotalCount)> GetPagedAsync(
            int pageNumber,
            int pageSize,
            int? productId = null,
        ...
        Task<bool> ExistsByProductAndTagAsync(int productId, string tagText, CancellationToken cancellationToken = default);
