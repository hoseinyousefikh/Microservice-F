...
namespace Catalog_Service.src._01_Domain.Core.Contracts.Repositories
{
    public interface IProductReviewRepository
    {
        ...
        Task<ProductReview> AddAsync(ProductReview review, CancellationToken cancellationToken = default);
        void Update(ProductReview review);
        void Remove(ProductReview review);
        Task<int> SaveChangesAsync(CancellationToken cancellationToken = default);

        // متدهای جستجو و فیلتر
        Task<(IEnumerable<ProductReview> Reviews, int TotalCount)> GetPagedAsync(
            int pageNumber,
            int pageSize,
            int? productId = null,
            string userId = null,
